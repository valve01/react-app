...
<!-- Также существует useContext. Он подобен addEventListner - отслеживает изменения контекста и перерисовывает компоненты, которые используют его контекст в случае его изменения. -->
