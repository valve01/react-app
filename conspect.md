...
<!-- Добавили add() -->
  ...
  123.86 kB (+21 B)  build\static\js\main.6a718ce0.js
  4.2 kB             build\static\css\main.68f91e30.css

<!-- ...20:23 -->


<!-- Чтобы сократить размер бандла:
1. Выносим часть JSкода (не компонент!) в отдельные файлы (вебпак сделает из него отдельный чанк) с помощью динамического импорта. Этот код будет загружаться после загрузки основного бандла
<!-- Например -->
<!-- 
...
<!-- Это то же самое что : (типа берем все экспорты, содержащиеся в файле math и запихиваем в переменную)
import * as math from '../utils/math'
 -->
<!-- Только загрузка файла math произойдет только в случае использования переменной math -->
<!-- Часто бывает полезно динамическим импортом загрузить библиотеку -->


    																														<!-- React.lazy -->
<!-- 2. Выносим страницы в отдельные чанки -->
<!-- Для этого используем React.lazy -->
<!-- Функция React.lazy позволяет рендерить динамический импорт как обычный компонент. -->
<!-- Динамический импорт компонента выглядит так -->
<!-- 
const OtherComponent = React.lazy(() => import('./OtherComponent'));
 -->

<!-- Идем в App.tsx и меняем импорты страниц на динамические -->
<!-- Например -->
<!-- const Cart = React.lazy(() => import('./pages/Cart')); -->

<!-- Теперь нужно обернуть Routes в компонент реакта Suspense, у которого есть атрибут fallback в который можно поместить то, что нужно показать пока идет загрузка-->
...
<!-- Либо можно обернуть только определенный компонент -->
<!-- 
		<Routes>
			<Route path="/" element={<MainLayout />}>
				<Route path="" element={<Home />} />
 ...
 <!-- Теперь код из Cart.tsx грузится отдельным чанком, при попытке перейти в корзину -->
<!-- При том что обычный импорт загружал был все файлы и импорты на всех страницах -->
...
<!-- Чтобы именовать чанк достаточно приписать перед путем :

/*webpackChunkName: "Cart"*/

<!-- где вместо Cart можно любое название сделать -->
...
<!-- Можно также сделать именование всех чанков на основе названий их файлов -->

<!-- 
// const FullPizzaPage="FullPizza"
// const FullPizza = React.lazy(() => import(/*webpackChunkName: "[request]"*/`./components/${FullPizzaPage}`));
 -->
<!-- где [request] -ключевое слово -->
<!-- благодаря такой конструкции каждый компонент станет отдельным чанком и проименуется динамически на основе своего имени файла. Не понятно как это работает -->
    																														<!-- react.loadable и loadable Components-->

<!-- react.loadable и loadable Components  это библиотеки для ленивого рендера на стороне сервера-->

<!-- 54:00 -->



<!-- Ставим библиотеку и типы для нее -->
<!-- npm i react-loadable -->
<!-- npm i @types/react-loadable -->

<!-- Меняем import из  React.lazy на loadable -->
...
<!-- свойство loading требует функциональный компонент, так что если мы просто хотим дать ему разметку - то заворачиваем ее в функцию -->
<!-- 
const Cart = Loadable({
	loader: () => import(/*webpackChunkName: "Cart"*/'./pages/Cart'),
...
<!-- </Suspense> не нужен при использовании  react.loadable и loadable Components - не работает, но и не мешает-->

    																														<!-- Именованный export в React.lazy  -->

<!-- React.lazy и react.loadable и loadable Components в настоящее время поддерживает только экспорт по умолчанию. Если модуль, который требуется импортировать, использует именованный экспорт, можно создать промежуточный модуль, который повторно экспортирует его как модуль по умолчанию. Это гарантирует работоспособность tree shaking — механизма устранения неиспользуемого кода. -->

<!-- Вот так это выглядит на практике на примере FullPizza. Но вот работает или нет - непонятно, т.к. чанк FullPizza не создается -->
<!-- 
const FullPizza = React.lazy(() =>
 ...
 -->
<!-- Потребляем промис, берем из него свойство module Мы по умолчанию передаем объект (который нам вернет then), в котором есть свойство default и в нем ,будет содержаться наш компонент -->
...
добавляем в package.json homepage после private

в нем у нас ссылка из GitHub Pages на этот репозиторий в GitHub

  "private": true,
  "homepage": "https://valve01.github.io/react-app-study/",

ставим пакет gh-pages

npm i -D gh-pages
он создаст на github отдельную ветку gh-pages, в которой будет хранится содержимое папки build. Эту ветку мы и будем деплоить. А gh-pages будет автоматически постоянно потом деплоить, когда мы будем комитить в main (ну или в другую ветку на которую настроим gh-pages)

возвращаемся в package.json

в scripts добавляем свойства

"deploy": "gh-pages -d build" // gh-pages - название утилиты, -d - дает команду deploy , build - это директория откуда брать файлы на деплой
"predeploy": "npm run build"


deploy - будем использовать только чтобы задеплоить на github pages
predeploy- мы не будем сами запускать, оно будет автоматически запускаться, чтобы перед деплоем заново собрать приложение
...
(авторизируемся если надо на github)

теперь если перейти по ссылке из github pages мы увидим документацию по нашему проекту из файла readme.md
и у нас появилась новая ветка в проекте: gh-pages

в ней как раз все файлы из build
А из main папка build пропала

теперь тут (https://github.com/valve01/react-app-study/settings/pages) меняем ветку, на основе которой формируется github pages c main на gh-pages.
<!-- ========================================================================================================================================== -->
Для SPA
меняем BrouserRouter на HashRouter в index.tsx
<!-- ========================================================================================================================================== -->
<!-- ========================================================================================================================================== -->
<!-- ========================================================================================================================================== -->
<!-- ========================================================================================================================================== -->
